...
get_weather_forecaset_by_cities <- function(city_names){
  ...
  for (city_name in city_names){
    ...
    api_key <- "ba9a92705abd9c02cad15f9e7c530dbd"
...
predict_bike_demand <- function(TEMPERATURE, HUMIDITY, WIND_SPEED, SEASONS, HOURS){
...
generate_city_weather_bike_data <- function (){
  ...
  results <- weather_df %>% 
    mutate(BIKE_PREDICTION=predict_bike_demand(TEMPERATURE, HUMIDITY, WIND_SPEED, SEASONS, HOURS)) %>%
